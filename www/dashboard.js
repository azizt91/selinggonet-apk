// dashboard.js (Supabase version)
        ...
        try {
          ...
          if (Capacitor.isNativePlatform()) {
            ...
            await PushNotifications.addListener('registration', async (token) => {
              ...
              if (user) {
                const { error } = await supabase.from('device_tokens').upsert({ user_id: user.id, token: token.value },
              ...
              try {
                  ...
                  if (profile) {
                      ...
                      if (profile.photo_url && userAvatar) {
                          ...
                          userAvatar.innerHTML = <span class="text-white text-xl font-bold flex items-center 
  justify-center h-full">${initials}</span>;
          ...
          function populateFilters() {
              const namaBulan = ["Semua Bulan", "Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", 
  "Agustus", "September", "Oktober", "November", "Desember"];
          ...
          async function fetchDashboardStats() {
              ...
              try {
                  ...
                  hideLoading();
                  hideChartsLoading();
                  cardsContainer.innerHTML = <p class="text-center text-red-500 col-span-full">Gagal memuat data: 
  ${error.message}</p>;
              }
          }
  ...
  '💰' },
                  { label: 'Pendapatan', value: formatter.format(stats.total_revenue || 0), gradient: 
  'gradient-card-2', icon: '📈' },
                  { label: 'Pengeluaran', value: formatter.format(stats.total_expenses || 0), gradient: 
  'gradient-card-3', icon: '💸' },
                  { label: 'Pelanggan Aktif', value: stats.active_customers || 0, gradient: 'gradient-card-4', icon: 
  '👥', link: activeCustomersLink },
                  { label: 'Pelanggan Tdk Aktif', value: stats.inactive_customers || 0, gradient: 'gradient-card-5', 
  icon: '😴', link: inactiveCustomersLink },
                  { label: 'Belum Dibayar', value: stats.unpaid_invoices_count || 0, gradient: 'gradient-card-6', icon:
   '⏳', link: unpaidLink },
                  { label: 'Lunas Bulan Ini', value: stats.paid_invoices_count || 0, gradient: 'gradient-card-7', icon:
   '✅', link: paidLink }
              ];
              ...
              try {
                  ...
                  if (revenueCtx && chartsData.revenue_chart) {
                      revenueChart = new Chart(revenueCtx, {
                          ...
                          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 
  'top', labels: { usePointStyle: true, padding: 20, font: { size: 12 }}}, tooltip: { mode: 'index', intersect: false, 
  callbacks: { label: function(context) { return context.dataset.label + ': Rp ' + new 
  Intl.NumberFormat('id-ID').format(context.parsed.y);}}}}, scales: { y: { beginAtZero: true, ticks: { callback: 
  function(value) { return 'Rp ' + new Intl.NumberFormat('id-ID', { notation: 'compact', compactDisplay: 'short' 
  }).format(value);}}}}, interaction: { mode: 'nearest', axis: 'x', intersect: false }}
                      });
                  }
                  ...
                  if (paymentStatusCtx && chartsData.payment_status_chart) {
                      paymentStatusChart = new Chart(paymentStatusCtx, {
                          ...
                          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 
  'bottom', labels: { padding: 20, usePointStyle: true, font: { size: 12 }}}, tooltip: { callbacks: { label: 
  function(context) { const total = context.dataset.data.reduce((a, b) => a + b, 0); const percentage = 
  ((context.parsed / total) * 100).toFixed(1); return context.label + ': ' + context.parsed + ' (' + percentage + 
  '%)';}}}}, cutout: '60%' }
                      });
                  }
                  ...
                  if (customerGrowthCtx && chartsData.customer_growth_chart) {
                      customerGrowthChart = new Chart(customerGrowthCtx, {
                          ...
                          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 
  'top', labels: { usePointStyle: true, padding: 20, font: { size: 12 }}}, tooltip: { callbacks: { label: 
  function(context) { return context.dataset.label + ': ' + context.parsed.y + ' pelanggan';}}}}, scales: { y: { 
  ...
  }, tooltip: { callbacks: { label: function(context) { return 'Total Aktif: ' + context.parsed.y + ' pelanggan';}}}}, 
  scales: { y: { beginAtZero: true, ticks: { stepSize: 5 }}}, interaction: { mode: 'nearest', axis: 'x', intersect: 
  false }}
                      });
                  }
